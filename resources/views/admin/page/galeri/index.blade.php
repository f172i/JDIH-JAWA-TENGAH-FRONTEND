                                ...
                                <div class="card card-flush h-xl-100">
                                    <div class="card-header">
                                        <h3 class="card-title align-items-start flex-column">
                                            <span class="card-label fw-bolder text-gray-800">Galeri</span>
                                        </h3>
                                        <div class="card-title align-items-end flex-column">
                                            <a href="#" class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                   			data-bs-target="#tambahvm"><i class="fa fa-plus"></i>Tambah Galeri</a>
                                        </div>
                                    </div>
                                            ...
                                            <tbody>
                                                ...
                                                @foreach($data as $d)
                                                    <tr>
                                                        <td></td>
                                                        <td>{{ $d->name }}</td>
                                                        <td>{{ $d->jenis == 1 ? "Gambar":"Video"; }}</td>
                                                        <td>@if($d->file != '')
                                                               <img style="max-width:200px;max-height:200px;" src="{{ asset('galeri/'.$d->file)}}"> 
                                                            @else
                                                                <iframe style="max-width:200px;max-height:200px;" src="{{$d->link}}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                                                            @endif
                                                        </td>
                                                        <td><a class="btn btn-sm btn-danger hapusb" id="{{$d->id}}">Hapus</a></td>
                                                    </tr>
                                                @endforeach
                                                @endif
                                            </tbody>
					...
					<div class="modal-body">
						<form id="form-filter">
							<div class="mb-3 row">
								...
								<div class="col-sm-12">
								    <input type="text" class="form-control" id="name" value="" placeholder="masukan nama Banner">
								</div>
							</div>
                            <div class="mb-3 row">
                            ...
                            <div class="mb-3 row">
								<label class="col-sm-12 col-form-label">File *Pilih salah satu input file / input link</label>
								<div class="col-sm-12">
								<input type="file" class="form-control" id="file" value="">
								</div>
							</div>
                            <div class="mb-3 row">
								<label class="col-sm-12 col-form-label">Link *Pilih salah satu input file / input link</label>
								<div class="col-sm-12">
								<input type="text" class="form-control" id="link" value="" placeholder="masukan kode Link embed youtube">
								</div>
							</div>
						</form>
					</div>
...
@section('script')
    ...
    $('#tambahphproses').on('click',function(){
        ...
        axios({
        method: "post",
        url: "{{ route('admin.master.galeri.store') }}",
        data: {
            name: $('#name').val(),
            jenis: $('#jenis').val(),
            link: $('#link').val(),
        ...
        .catch(function (response) {
            ...
            Swal.fire({
                icon: 'error',
                title:'gagal tambah galeri baru'
            })		
        });
    });
</script>
    	...
    	$('#datatables tbody').on('click', '.hapusb', function () {
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
			    ...
			    Swal.showLoading()
                $.ajax({
                  url: '{{ route('admin.master.galeri.delete') }}',
                  type: 'post',
                  data: {
					id:id,
					_token: "{{ csrf_token() }}"
