						...
						<div class="toolbar" id="kt_toolbar">
							...
							<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
								...
								<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
									<!--begin::Title-->
									<h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">Data Master Pelayanan Hukum Elektronik</h1>
									<!--begin::Separator-->
                                ...
                                <div class="card card-flush h-xl-100">
                                    <div class="card-header">
                                        <h3 class="card-title align-items-start flex-column">
                                            <span class="card-label fw-bolder text-gray-800">Data Pelayanan Hukum Elektronik</span>
                                        </h3>
                                    ...
                                    <div class="card-body pt-2">
                                        <table class="table table-bordered table-hover w-100" id="datatables">
                                            <thead>
                                                <tr>
                                                    <th>No</th>
                                                    <th>Nama Pelayanan Hukum Elektronik</th>
                                                    <th>Link</th>
													<th>Aksi</th>
                                                </tr>
                                            </thead>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header text-center">
						<h5 class="modal-title" id="exampleModalLabel">Tambah Pelayanan Hukum Elektronik</h5>
					...
					<div class="modal-body">
						<form id="form-filter">
							<div class="mb-3 row">
								...
								<div class="col-sm-12">
								<input type="text" class="form-control" id="name" value="" placeholder="masukan nama Pelayanan Hukum Elektronik">
								</div>
							</div>
                            <div class="mb-3 row">
								<label class="col-sm-12 col-form-label">Logo</label>
								<div class="col-sm-12">
								<input type="file" class="form-control" id="logo" value="" placeholder="masukan logo Pelayanan Hukum Elektronik">
								</div>
							</div>
                            <div class="mb-3 row">
								<label class="col-sm-12 col-form-label">Link</label>
								<div class="col-sm-12">
								<input type="text" class="form-control" id="link" value="" placeholder="masukan link Pelayanan Hukum Elektronik">
								</div>
							</div>
						</form>
					</div>
...
@section('script')
    ...
    <script>
       $('#datatables').DataTable({
            ...
            serverSide: false,
            ajax: {
                url: '{{ route('admin.master.ph.datatable') }}',
                type: 'post',
            ...
            columns: [
                ...
                {data: 'link', orderable: true, searchable: true},
    ...
    <script type="text/javascript">
    	$('#datatables tbody').on('click', '.hapusph', function () {
            let id = this.id;
            Swal.fire({
              ...
              if (result.isConfirmed) {
			    ...
			    Swal.showLoading()
                $.ajax({
                  url: '{{ route('admin.master.ph.delete') }}',
                  type: 'post',
                  data: {
					id:id,
					_token: "{{ csrf_token() }}"
...
<script>
    $('#tambahphproses').on('click',function(){
        ...
        axios({
        method: "post",
        url: "{{ route('admin.master.ph.store') }}",
        data: {
            name: $('#name').val(),
            link: $('#link').val(),
            logo: $('#logo')[0].files[0],
        ...
        .catch(function (response) {
            ...
            Swal.fire({
                icon: 'error',
                title:'gagal tambah pelayanan hukum elektronik baru'
            }).then(() => {
                // location.reload();
            })		
        });
    ...
    $('document').ready(function(){
        function getEdit(id) {
         $('#body-edit').html('<div class="card"><h2 style="text-align: center;">Loading...</h2></div>');
			var url = `{{ url('/admin/master-pelayanan-hukum-update') }}/${id}`;
            $.get(url, function(data) {
                $('#body-edit').html(data);
            });
        }
		$('#datatables tbody').on('click', '.editph', function () {
            var id = this.id;
            getEdit(id)
        })
